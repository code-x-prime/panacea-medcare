...
export default function AyurvedaPage({ params }) {
    const { locale } = params;
    const t = useTranslations("treatmentsAyurveda");
    const isRTL = locale === "ar";

    const breadcrumbItems = [
        { label: t("breadcrumb.home"), href: `/${locale}` },
        { label: t("breadcrumb.treatments"), href: `/${locale}/treatments` },
        { label: t("breadcrumb.ayurveda"), href: `/${locale}/treatments/ayurveda` }
    ];

    const treatments = [
        { title: t("treatments.panchakarma.title"), description: t("treatments.panchakarma.description") },
        { title: t("treatments.arthritis.title"), description: t("treatments.arthritis.description") },
        { title: t("treatments.chronicPain.title"), description: t("treatments.chronicPain.description") },
        { title: t("treatments.digestive.title"), description: t("treatments.digestive.description") },
        { title: t("treatments.diabetes.title"), description: t("treatments.diabetes.description") },
        { title: t("treatments.stress.title"), description: t("treatments.stress.description") },
        { title: t("treatments.integrative.title"), description: t("treatments.integrative.description") },
        { title: t("treatments.wellness.title"), description: t("treatments.wellness.description") }
    ];
    ...
    return (
        <main dir={isRTL ? "rtl" : "ltr"} className="bg-white">
            ...
            <section className="container mx-auto px-4 sm:px-6 lg:px-8 py-8"><Breadcrumb items={breadcrumbItems} locale={locale} /></section>
            <section className="bg-gradient-to-br from-lime-50 via-white to-lime-50 py-12 md:py-16">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="max-w-4xl mx-auto text-center">
                        <div className="inline-flex items-center justify-center w-20 h-20 bg-panacea-primary/10 rounded-full mb-6">
                            <Image
                                src="/treatment/ayurveda-holistic-wellness.svg"
                                alt="Ayurveda"
                                width={40}
                                height={40}
            ...
            <section className="bg-panacea-light py-16 md:py-20">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="max-w-4xl mx-auto">
                        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-panacea-dark mb-12 text-center break-words" style={{ wordBreak: 'break-word', hyphens: 'auto', lineHeight: '1.3' }}>{t("faqs.title")}</h2>
                        <div className="space-y-6">
                            {faqs.map((faq, index) => (
                                <div key={index} className="bg-white p-6 md:p-8 rounded-xl shadow-panacea">
                                    <h3 className="text-base sm:text-lg md:text-xl font-bold text-panacea-dark mb-3 break-words" style={{ wordBreak: 'break-word', hyphens: 'auto', lineHeight: '1.4' }}>{faq.question}</h3>
                                    <p className="text-sm sm:text-base text-panacea-gray leading-relaxed break-words" style={{ wordBreak: 'break-word', hyphens: 'auto', lineHeight: '1.6' }}>{faq.answer}</p>
                                </div>
                            ))}
                        </div>
                    </div>
